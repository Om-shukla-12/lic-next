    ...
    return (
        <div className="min-h-screen bg-[#fcfdff] relative overflow-hidden">
            ...
            <main className="relative z-10">
                {/* Header Section */}
                {/* Header Section */}
                <div className="relative z-10 p-4 md:p-6 max-w-[100rem] mx-auto space-y-4 md:space-y-6">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-2">
                        <div className="space-y-2 w-full max-w-2xl">
                            <Link href="/agent-dashboard" className="inline-flex items-center gap-2 py-1.5 px-4 bg-white/60 backdrop-blur-md border border-slate-200 rounded-full text-[10px] font-black text-blue-600 shadow-sm hover:shadow-md transition-all hover:-translate-x-1 uppercase tracking-widest w-fit">
                                <ChevronLeft className="w-3.5 h-3.5" /> Back to Dashboard
                            </Link>

                            <div className="flex items-center justify-between md:block">
                                <h1 className="text-2xl md:text-5xl font-black text-slate-900 tracking-tighter flex flex-wrap items-center gap-x-2 md:gap-x-4 whitespace-nowrap">
                                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-700 via-indigo-600 to-violet-600">
                                        Member's
                                    </span>
                                    <span className="flex items-center gap-2">
                                        Birthdays <Cake className="w-8 h-8 md:w-12 md:h-12 text-rose-500 animate-pulse" />
                                    </span>
                                </h1>

                                {/* Mobile Actions Row (Aligned with Title) */}
                                <div className="flex items-center gap-2 md:hidden">
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className={`h-9 w-9 text-slate-500 ${isSearchOpen ? 'text-blue-600 bg-blue-50' : ''}`}
                                        onClick={() => {
                                            setIsSearchOpen(!isSearchOpen);
                                            if (!isSearchOpen) setTimeout(() => document.getElementById('bd-search-mobile')?.focus(), 100);
                                        }}
                                    >
                                    ...
                                    <Button
                                        variant="default"
                                        size="icon"
                                        className="h-9 w-9 rounded-xl bg-white hover:bg-slate-50 text-blue-600 shadow-lg shadow-blue-100/50 border border-slate-100 shrink-0"
                                        onClick={refresh}
                                        disabled={isLoading}
                                    >
                        ...
                        <div className="hidden md:flex items-center gap-4 self-end w-auto justify-end">
                    ...
                    {/* Filter Section & Mobile Search Input */}
                    <div className="w-full md:w-fit space-y-3">
                        {/* Mobile Expanded Search Bar - Appears above Dropdown */}
                        {isSearchOpen && (
                            <div className="md:hidden relative animate-in fade-in slide-in-from-top-2 duration-200">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <Input
                                    id="bd-search-mobile"
                                    placeholder="Search names..."
                                    className="pl-11 h-12 rounded-xl border-none bg-white shadow-lg shadow-blue-100/50 focus:ring-2 focus:ring-blue-500/20 text-slate-700 font-bold text-sm"
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                />
                            </div>
                        ...
                        <div className="md:hidden relative">
                            <select
                                ...
                                className="w-fit min-w-[140px] appearance-none bg-white/80 backdrop-blur-md border border-slate-200 rounded-lg py-2 pl-3 pr-8 text-xs font-black uppercase tracking-wider text-slate-600 shadow-sm focus:ring-2 focus:ring-blue-500/20 outline-none"
                            >
                                <option value="today">Today</option>
                                <option value="week">This Week</option>
                                <option value="yesterday">Yesterday</option>
                            </select>
                            <div className="absolute left-[min(100%,140px)] -ml-6 top-1/2 -translate-y-1/2 pointer-events-none">
                                <ChevronLeft className="w-3 h-3 rotate-270 text-slate-400" />
                            </div>
                        </div>
                        {/* <select
                            value={filter}
                            onChange={(e) => setFilter(e.target.value)}
                            className="w-full appearance-none bg-white/80 backdrop-blur-md border border-slate-200 rounded-xl py-3 px-4 text-slate-700 font-bold shadow-sm focus:ring-2 focus:ring-blue-500/20 outline-none"
                        >
                            <option value="today">Today's Birthdays</option>
                            <option value="week">This Week</option>
                            <option value="yesterday">Yesterday</option>
                        </select>
                        <ChevronLeft className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 rotate-270 text-slate-400 pointer-events-none" /> */}
                    </div>

                    {/* Desktop Tabs */}
                    <div className="hidden md:flex p-1 bg-white/50 backdrop-blur-md rounded-2xl border border-white/50 shadow-sm w-fit mx-auto md:mx-0">
